...
def generate_vec_stub(name):
    ...
    class {name}:{''.join(f'''
        ...
        {''.join(f'''
        @property
        def {swizzle}(self) -> {vector.data_type}{vector.data_size if vector.data_type not in 'bd' else ''}vec{len(swizzle)}: ...
        @{swizzle}.setter
        ...
        {f'''
        def __and__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __rand__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __iand__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...

        def __or__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __ror__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __ior__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...

        def __xor__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __rxor__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __ixor__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...'''
        if vector.data_type in 'bi' else ''}
        {f'''
        def __lshift__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __rlshift__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __ilshift__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...

        def __rshift__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __rrshift__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...
        def __irshift__(self, other: {union([this_vector_union, 'glm_typing.Number'])}) -> {name}: ...'''
        if vector.data_type == 'i' else ''}
